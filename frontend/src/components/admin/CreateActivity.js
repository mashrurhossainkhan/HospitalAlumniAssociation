...
import { createActivity } from '../../api/apiAdmin';
...
const CreateActivity = () => {
    ...
    const handleSubmit = (e) => {
        ...
        createActivity(token, { name: name })
            .then(response => {
                setValues({
                    ...values,
                    error: false,
    ...
    const activityForm = () => {
        return (
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn btn-outline-primary">Create Activity</button>
            </form>
        )
    }
    ...
    return (
        <Layout title="Add a new Activity">
            <div className="row">
                <div className="col-md-8 offset-md-2">
                    {showLoading(loading)}
                    {showError(error, error)}
                    {showSuccess(success, 'Activity Created!')}
                    {activityForm()}
                    {goBack()}
                </div>
            </div>
